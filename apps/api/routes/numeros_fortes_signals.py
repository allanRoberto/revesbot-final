...
router = APIRouter()
signals_coll = mongo_db["numeros_fortes_signals"]
live_coll = mongo_db["numeros_fortes_live"]
...
def _serialize(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "gatilho":           doc.get("gatilho"),
        "gatilho_weight":    doc.get("gatilho_weight", 0),
        "gatilho_fanout":    doc.get("gatilho_fanout", 0),
        "numeros_fortes":    doc.get("numeros_fortes", []),
        "gatilhos_snapshot": doc.get("gatilhos_snapshot", []),
        "bet":               doc.get("bet", []),
        "fortes_count":      doc.get("fortes_count", 0),
...
@router.get("/api/numeros-fortes-signals")
...
@router.get("/api/numeros-fortes-live")
...
@router.get("/api/numeros-fortes-signals/{signal_id}")
