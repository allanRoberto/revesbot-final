...
router = APIRouter()
signals_coll = mongo_db["puxadas_inversao_signals"]
...
def _serialize(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "prev_ranking":    doc.get("prev_ranking", []),
...
@router.get("/api/puxadas-inversao/signals")
async def list_signals(
    ...
    base = {k: v for k, v in filt.items() if k != "status"}
    ...
    # Acompanhamento pós: média de hits/10 rodadas
    post_hits_list: List[int] = []
    async for d in signals_coll.find(
        {**base, "post_tracking.completed": True},
        {"post_tracking.hits": 1}
    ):
        h = (d.get("post_tracking") or {}).get("hits", 0)
        post_hits_list.append(int(h))
    avg_post_hits = round(sum(post_hits_list) / len(post_hits_list), 2) if post_hits_list else None
...
@router.get("/api/puxadas-inversao/signals/{signal_id}")
