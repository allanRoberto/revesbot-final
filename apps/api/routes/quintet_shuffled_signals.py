...
router = APIRouter()
signals_coll = mongo_db["quintet_shuffled_signals"]
...
def _serialize(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Ranking: top items for display
    ranking = doc.get("ranking") or []
    return {
        "id":                  str(doc["_id"]),
        "roulette_id":         doc.get("roulette_id", ""),
        "status":              doc.get("status", ""),
        "quintet":             doc.get("quintet", []),
        "quintet_raw":         doc.get("quintet_raw", []),
        "top_bet":             doc.get("top_bet", 18),
        "bet":                 doc.get("bet", []),
        "ranking":             ranking[:20],   # top-20 para exibição
...
@router.get("/api/quintet-shuffled-signals")
...
@router.get("/api/quintet-shuffled-signals/{signal_id}")
