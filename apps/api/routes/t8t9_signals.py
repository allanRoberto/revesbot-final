...
router = APIRouter()
signals_coll = mongo_db["t8t9_signals"]
...
def _serialize(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "trigger_val":         doc.get("trigger_val"),
        "trigger_terminal":    doc.get("trigger_terminal"),
        "last_10":             doc.get("last_10", []),
        "cluster_count":       doc.get("cluster_count", 0),
        "has_strong_cluster":  bool(doc.get("has_strong_cluster", False)),
        "clusters":            doc.get("clusters", []),
        "bet":                 doc.get("bet", []),
        "multipliers":         doc.get("multipliers", []),
        "payout":              doc.get("payout", 36),
...
@router.get("/api/t8t9-signals")
async def list_signals(
    roulette_id:        Optional[str]  = Query(None),
    status:             Optional[str]  = Query(None),
    has_strong_cluster: Optional[bool] = Query(None),
    ...
    if status and status != "all":
        filt["status"] = status
    if has_strong_cluster is not None:
        filt["has_strong_cluster"] = has_strong_cluster

    base_filt = {k: v for k, v in filt.items() if k not in ("status", "has_strong_cluster")}
    ...
    # Breakdown: forte vs fraco
    strong_counts: Dict[str, Any] = {}
    for strong_val, label in ((True, "forte"), (False, "fraco")):
        sf = {**base_filt, "has_strong_cluster": strong_val}
        sw = await signals_coll.count_documents({**sf, "status": "won"})
        sl = await signals_coll.count_documents({**sf, "status": "lost"})
        sm = await signals_coll.count_documents({**sf, "status": "monitoring"})
        sr = sw + sl
        strong_counts[label] = {
            "total": sw + sl + sm, "won": sw, "lost": sl, "monitoring": sm,
            "assertiveness": round(sw / sr * 100, 2) if sr > 0 else 0.0,
        }

    return {
        "filters": {
            "roulette_id": roulette_id, "status": status,
            "has_strong_cluster": has_strong_cluster,
        },
        "total":               total,
        "won":                 won,
        "lost":                lost,
        ...
        "strong_breakdown":    strong_counts,
        "inversion": {
            "paid":     inversion_paid,
...
@router.get("/api/t8t9-signals/{signal_id}")
