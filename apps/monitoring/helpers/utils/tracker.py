...
class HotColdTracker:
    ...
    def add_spin(self, number: int) -> None:
        ...
        neighbords = get_neighbords(number)
        neighbords_color = get_neighbords_color(number)
        self.counts[neighbords[0]] += 0.9
        self.counts[neighbords[1]] += 0.9

        self.counts[neighbords_color[0]] += 0.8
        self.counts[neighbords_color[1]] += 0.8
    ...
    def bulk_add(self, spins: List[int]) -> None:
        ...
        for n in spins:
            if n not in(0, 2, 4, 5, 6, 7, 9, 13, 16, 17, 19, 21, 34, 27, 22, 25,28, 29, 33):
                continue
            else :
                self.add_spin(n)
    ...
    def top_n(self, n: int = 5) -> List[Tuple[int, int]]:
        """Retorna os n números mais quentes, apenas os números."""
        return [num for num, _ in self.counts.most_common(n)]
