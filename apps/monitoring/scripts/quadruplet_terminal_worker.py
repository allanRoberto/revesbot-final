...
ROULETTE_IDS_RAW   = (
    os.getenv("QUAD_TERMINAL_ROULETTE_IDS")
    or "pragmatic-auto-roulette,pragmatic-brazilian-roulette"
)
SPINS_TO_FETCH     = int(os.getenv("QUAD_TERMINAL_SPINS",        "500"))
PRE_WINDOW         = int(os.getenv("QUAD_TERMINAL_PRE_WINDOW",   "4"))   # 4 spins exibidos no pré
MAX_ATTEMPTS       = int(os.getenv("QUAD_TERMINAL_MAX_ATTEMPTS", "4"))
MAX_POST_TRACK     = int(os.getenv("QUAD_TERMINAL_MAX_POST",     "20"))
MIN_OCCURRENCES    = int(os.getenv("QUAD_TERMINAL_MIN_OCCUR",    "3"))
MIN_SCORE          = int(os.getenv("QUAD_TERMINAL_MIN_SCORE",    "3"))
NEIGHBOR_SPAN      = int(os.getenv("QUAD_TERMINAL_NEIGHBOR_SPAN","1"))
POSITIONS          = 3   # top-3 ranking — fixo
POLL_SECONDS       = float(os.getenv("QUAD_TERMINAL_POLL_SECONDS", "2"))

_blocked_raw = os.getenv("QUAD_TERMINAL_BLOCKED_TERMINALS", "").strip()
...
def try_generate_signal(spins: List[Dict]) -> Optional[Dict[str, Any]]:
    # Need at least 4 spins to have prev1, a, b, c
    if len(spins) < 4:
        return None

    # Quadruplet key: last 4 spins
    # prev1 = spins[-4], a = spins[-3], b = spins[-2], c = spins[-1]
    prev1 = spins[-4]["value"]
    a     = spins[-3]["value"]
    b     = spins[-2]["value"]
    c     = spins[-1]["value"]
    ...
    total_occ, top3 = query_quadruplet_ranking(prev1, a, b, c)
    if not top3:
        return None
...
def create_signal(rid: str, info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    doc = {
        "roulette_id":        rid,
        "status":             "monitoring",
        "quadruplet":         [info["prev1"], info["a"], info["b"], info["c"]],
        "triplet":            [info["a"], info["b"], info["c"]],   # compat display
        "top3":               info["top3"],
        ...
        "config": {
            ...
            "positions":    POSITIONS,
            "key_length":   4,  # identifica que usa 4-number key
        },
    }
    result = signals_coll.insert_one(doc)
    doc["_id"] = result.inserted_id
    log.info(
        "[%s] SIGNAL quad=%s top3=%s terminal=%d strength=%s score=%d bet_size=%d inv=%s",
        rid, [info["prev1"], info["a"], info["b"], info["c"]], info["top3"],
...
def main() -> None:
    ...
    if not targets:
        log.error("Nenhuma roleta para monitorar. QUAD_TERMINAL_ROULETTE_IDS=%s", ROULETTE_IDS_RAW)
        return

    log.info(
        "Quadruplet worker iniciado. Roletas=%d max_attempts=%d min_occur=%d min_score=%d neighbor_span=%d poll=%ss",
