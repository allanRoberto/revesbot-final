...
SPINS_TO_FETCH  = int(os.getenv("TRIPLET_TERMINAL_SPINS",        "500"))
PRE_WINDOW      = int(os.getenv("TRIPLET_TERMINAL_PRE_WINDOW",   "3"))
MAX_ATTEMPTS    = int(os.getenv("TRIPLET_TERMINAL_MAX_ATTEMPTS", "4"))
MAX_POST_TRACK  = int(os.getenv("TRIPLET_TERMINAL_MAX_POST",     "20"))
...
def try_generate_signal(spins: List[Dict]) -> Optional[Dict[str, Any]]:
    ...
    # The trigger triplet is the last 3 spins before the current one
    # so we look at spins[-4], spins[-3], spins[-2] as (a,b,c) trigger state
    # and spins[-1] is the "current" spin that fired the evaluation
    # Actually: after the latest spin arrives, the triplet to analyze is the last 3 spins
    # (spins[-3], spins[-2], spins[-1]) = (a, b, c)
    a = spins[-3]["value"]
    b = spins[-2]["value"]
    c = spins[-1]["value"]
    trigger_ts = spins[-1]["timestamp"]

    total_occ, top3 = query_triplet_ranking(a, b, c)
    if not top3:
        return None
...
def create_signal(rid: str, info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    log.info(
        "[%s] SIGNAL triplet=%s top3=%s terminal=%d strength=%s score=%d bet_size=%d inv=%s",
        rid, [info["a"], info["b"], info["c"]], info["top3"],
...
def main() -> None:
    ...
    if not targets:
        log.error("Nenhuma roleta para monitorar. TRIPLET_TERMINAL_ROULETTE_IDS=%s", ROULETTE_IDS_RAW)
        return
