...
`pragmatic-auto-roulette`, editar o histórico e seis grupos, e enviar manualmente uma única
requisição ao Jev pelo OpenRouter.
...
As três rotas exigem HTTP Basic. O POST também exige o token CSRF emitido ao abrir a página.
