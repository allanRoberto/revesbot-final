#!/usr/bin/env python3
"""Busca 3000 sugestões por roleta e analisa a posição do ranking onde bateu."""
...
def correlate(db, roulette_id: str, limit: int = 3000):
    ...
    for snap in snapshots:
        ...
        # Extract ranking from payload.ordered_suggestion
        payload = snap.get("payload", {})
        ranking_full = payload.get("ordered_suggestion") or payload.get("suggestion") or []
...
def analyze_ranking_distribution(items: list) -> dict:
    ...
    stats = {
        ...
        "avg_rank": sum(r * rank_distribution[r] for r in ranks) / total_hits if total_hits > 0 else None,
...
def main():
    ...
    for r, data in summary["by_roulette"].items():
        ...
        if "analysis" in data and "statistics" in data["analysis"]:
            ...
            if avg_rank:
                print(f"      Média de ranking: {avg_rank:.2f}", flush=True)
